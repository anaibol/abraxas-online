...
'is not to be confused with a CallBack control, The only PUBLIC FUNCTION that uses
'callbacks is WSASetBlockingHook()
'(4) Alot of "helper" functions are provided in the file for various things
'before attempting to figure out how to call a PUBLIC FUNCTION, look and see if
'there is already a helper PUBLIC FUNCTION for it.
...
Type FLOWSPEC
    ...
    ServiceNpcType         As Integer  'Guaranteed, Predictive,
                                    'Best Effort, etc.
...
'this PUBLIC FUNCTION should work on 16 and 32 bit systems
#If Win16 Then
    Public Function ConnectSock(ByVal Host$, ByVal Port%, retIpPort$, ByVal HWndToMsg%, ByVal Async%) As Integer
...
#End If
    ...
    'integer, so this PUBLIC FUNCTION returns an integer for a string
    'just to keep an error from happening, it converts the
    'number to a negative if needed
    If Val(PortStr$) > Max_Integer_Value Then
...
#If Win16 Then
    Public Function GetProtocolByName(ByVal Protocol$) As Integer
    Dim tmpShort%
#ElseIf Win32 Then
...
#If Win16 Then
    Public Function GetServiceByName(ByVal service$, ByVal Protocol$) As Integer
    Dim Serv%
#ElseIf Win32 Then
...
'this PUBLIC FUNCTION DOES work on 16 and 32 bit systems
#If Win16 Then
...
'this PUBLIC FUNCTION DOES work on 16 and 32 bit systems
Public Function IpToAddr(ByVal AddrOrIP$) As String
    On Error Resume Next
...
Public Function IrcGetAscIp(ByVal IPL$) As String
    'this PUBLIC FUNCTION is IRC specific, it expects a long Ip stored in Network byte order, in a string
...
Public Function IrcGetLongIp(ByVal AscIp$) As String
    'this PUBLIC FUNCTION converts an ascii Ip string into a long Ip in network byte order
