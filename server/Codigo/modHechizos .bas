...
Dim DruidManaBonus As Single
    ...
    With UserList(UserIndex)
        ...
        If .clase = eClass.Druid Then
            If .Invent.AnilloEqpObjIndex = FLAUTAMAGICA Then
...
Sub LanzarHechizo(SpellIndex As Integer, UserIndex As Integer)
'***************************************************
'Autor: Unknown (orginal version)
'Last Modification: 17/11/2009
...
With UserList(UserIndex)
    ...
    HechizoIndex = .Stats.UserHechizos(SpellIndex)
    
    If PuedeLanzar(UserIndex, HechizoIndex) Then
        Select Case Hechizos(HechizoIndex).Target
            Case TargetType.uUsuarios
                If .flags.TargetUser > 0 Then
                    If Abs(UserList(.flags.TargetUser).Pos.Y - .Pos.Y) <= RANGO_VISION_Y Then
                        Call HandleHechizoUsuario(UserIndex, HechizoIndex)
                    Else
            ...
            Case TargetType.uNPC
                If .flags.TargetNPC > 0 Then
                    If Abs(Npclist(.flags.TargetNPC).Pos.Y - .Pos.Y) <= RANGO_VISION_Y Then
                        Call HandleHechizoNPC(UserIndex, HechizoIndex)
                    Else
                        Call WriteConsoleMsg(UserIndex, "Estás demasiado lejos para lanzar este hechizo.", FontTypeNames.FONTTYPE_WARNING)
                    End If
                Else
            ...
            Case TargetType.uTerreno
                Call HandleHechizoTerreno(UserIndex, HechizoIndex)
        End Select
...
With UserList(UserIndex)
    HechizoIndex = .Stats.UserHechizos(.flags.Hechizo)
    ...
    If Hechizos(HechizoIndex).Invisibilidad = 1 Then
        ...
        If (TriggerZonaPelea(UserIndex, TargetIndex) <> TRIGGER6_PERMITE) Then
            If criminal(TargetIndex) And Not criminal(UserIndex) Then
                If esArmada(UserIndex) Then
                    Call WriteConsoleMsg(UserIndex, "Los miembros del ejército real no pueden ayudar a los criminales.", FontTypeNames.FONTTYPE_INFO)
                    HechizoCasteado = False
                    Exit Sub
                End If
        ...
        'Call SendData(SendTarget.ToPCArea, tU, PrepareMessageSetInvisible(UserList(TargetIndex).Char.CharIndex, True))
        ...
        If (TriggerZonaPelea(UserIndex, TargetIndex) <> TRIGGER6_PERMITE) Then
            If criminal(TargetIndex) And Not criminal(UserIndex) Then
                ...
                If .flags.Seguro Then
                    ...
                    Exit Sub
                Else
                    Call DisNobAuBan(UserIndex, .Reputacion.NobleRep * 0.5, 10000)
                End If
            End If
        End If
    ...
    If Hechizos(HechizoIndex).Revivir = 1 Then
        If UserList(TargetIndex).flags.Muerto = 1 Then
            ...
            ElseIf .clase = eClass.Bard Then
                If .Invent.AnilloEqpObjIndex <> LAUDMAGICO Then
...
Sub HechizoPropUsuario(ByVal UserIndex As Integer, ByRef HechizoCasteado As Boolean)
'***************************************************
'Autor: Unknown (orginal version)
...
With UserList(tempChr)
    ...
    If Hechizos(SpellIndex).SubeMana = 1 Then
        ...
        If UserIndex <> tempChr Then
            ...
            Call WriteConsoleMsg(tempChr, UserList(UserIndex).name & " te ha restaurado " & daño & " puntos de maná.", FontTypeNames.FONTTYPE_FIGHT)
        Else
    ...
    ElseIf Hechizos(SpellIndex).SubeMana = 2 Then
        ...
        If UserIndex <> tempChr Then
            ...
            Call WriteConsoleMsg(tempChr, UserList(UserIndex).name & " te ha quitado " & daño & " puntos de maná.", FontTypeNames.FONTTYPE_FIGHT)
        Else
    ...
    If Hechizos(SpellIndex).SubeSta = 1 Then
        ...
        If UserIndex <> tempChr Then
            ...
            Call WriteConsoleMsg(tempChr, UserList(UserIndex).name & " te ha restaurado " & daño & " puntos de energía.", FontTypeNames.FONTTYPE_FIGHT)
        Else
    ...
    ElseIf Hechizos(SpellIndex).SubeSta = 2 Then
        ...
        If UserIndex <> tempChr Then
            ...
            Call WriteConsoleMsg(tempChr, UserList(UserIndex).name & " te ha quitado " & daño & " puntos de energía.", FontTypeNames.FONTTYPE_FIGHT)
        Else
