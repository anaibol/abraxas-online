...
'Date: 25 August 2009
'Description: Added FullScreenCapture Public function.
'
'
'Copyright.
...
Dim hFile As Long
Dim bgeneral_file_existsed As Boolean
Dim lFileSize As Long
   ...
   If lR = IJL_OK Then
      ...
      bgeneral_file_existsed = (FindFirstFile(sFile, tFnd) <> -1)
      If bgeneral_file_existsed Then
         Kill sFile
      End If
...
On Error GoTo Err:
    ...
    '    frmScreenshots.Picture1.Width = frmMain.Width
    '    frmScreenshots.Picture1.Height = frmMain.Height
    '    Call BitBlt(frmScreenshots.Picture1.hdc, 0, 0, 800, 600, hdcc, 0, 0, SRCCOPY)
    'Else
        'frmScreenshots.Picture1.Top = frmMain.Top '+ 30
        'frmScreenshots.Picture1.Left = frmMain.Left '+ 3
        'frmScreenshots.Picture1.Width = frmMain.Width '- 30
        'frmScreenshots.Picture1.Height = frmMain.Height '- 3
    ...
    dirFile = IIf(Autofragshooter, "/Fotos/Muerte", "/Fotos")
    
    If Not FileExist(App.path & dirFile, vbDirectory) Then
        MkDir (App.path & dirFile)
    End If
    ...
    ShowConsoleMsg "Screen Capturada!", 255, 255, 255, False, False, False
Exit Sub
...
'Medio desprolijo donde pongo la pic, pero es lo que hay por ahora
    ...
    If Not FileExist(App.path & "\TEMP", vbDirectory) Then
        MkDir (App.path & "\TEMP")
    End If
